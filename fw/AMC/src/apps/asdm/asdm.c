 ...
 *
 * This file contains the ASDM (AMR Data Store Model) implementation details
 *
 * @file asdm.c
 */
...
#define ASDM_HEADER_DEFAULT_BYTES ( sizeof( ASDMSensorHeader ) + ASDM_EOR_BYTES )
...
/**
 * @struct ASDMFptRecord
 * @brief  the fpt header and records
 */
typedef struct
{
    ASDMFptHeader xFptHdrPrimary;
    ASDMFptEntry  *pxFptEntryPrimary;
    ASDMFptHeader xFptHdrSecondary;
    ASDMFptEntry  *pxFptEntrySecondary;

} ASDMFptRecord;

/**
 * @struct  ASDMRecordField
...
/**
 * @struct ASDMBoardInfoRecord
 * @brief  the board info record
 */
typedef struct
{
    ASDMRecordField xEepromVersion;       /* ASCII */
    ASDMRecordField xProductName;         /* ASCII */
    ASDMRecordField xBoardRev;            /* ASCII */
    ASDMRecordField xBoardSerial;         /* ASCII */
    ASDMRecordField xMacAddressCount;     /* Literal/Binary */
    ASDMRecordField xFirstMacAddress;     /* Literal/Binary */
    ASDMRecordField xActiveState;         /* ASCII */
    ASDMRecordField xConfigMode;          /* Literal/Binary */
    ASDMRecordField xManufacturingDate;   /* Literal/Binary */
    ASDMRecordField xPartNumber;          /* ASCII */
    ASDMRecordField xUuid;                /* Literal/Binary */
    ASDMRecordField xPcieId;              /* Literal/Binary */
    ASDMRecordField xMaxPowerMode;        /* Literal/Binary */
    ASDMRecordField xMemorySize;          /* ASCII */
    ASDMRecordField xOemId;               /* Literal/Binary */
    ASDMRecordField xCapability;          /* Literal/Binary */
    ASDMRecordField xMfgPartNumber;       /* ASCII */

} ASDMBoardInfoRecord;

/**
 * @struct ASDMSdrRecord
 * @brief  a single SDR sensor record
 */
typedef struct
{
    uint8_t         ucId;                 /* uniquely identifies each sensor record within the SDR */
    ASDMRecordField xSensorName;
    ASDMRecordField xSensorValue;
    ASDMRecordField xSensorBaseUnit;
    char            cUnitModifier;
...
typedef struct
{
    ASDMSensorHeader xHdr;
    union
    {
        ASDMSdrRecord        *pxSensorRecord;
        ASDMFptRecord        xFptRecord;
...
{
    ASDMSensorHeader   xHdr;
    ASDMSdsRecord      *pxSensorRecord;
    uint8_t            pucAsdmEor[ASDM_EOR_BYTES];

} ASDMSds;

/**
 * @struct  ASDMPrivateData
...
static ASDMPrivateData xLocalData =
{
...
static ASDMPrivateData *pxThis = &xLocalData;
 ...
 static int iCalculateTotalPower( void )
 {
         ...
         ( NULL != pxThis->pxAsdmSdrInfo ) )
     {
         ...
         for ( iSensorIdx = 0; iSensorIdx < ucTotalNumRecords; iSensorIdx++ )
         {
             ...
             ASDMSdrRecord *pxSensorRecord = pxThis->pxAsdmSdrInfo[AMC_ASDM_SUPPORTED_REPO_POWER].pxSensorRecord;
         ...
         ( NULL != pxThis->pxAsdmSdrInfo ) )
     {
                                                   ...
                                                   OSAL_TIMEOUT_WAIT_FOREVER ) )
         {
             ...
             for ( iRepoIdx = 0; iRepoIdx < AMC_ASDM_SUPPORTED_REPO_TOTAL_POWER; iRepoIdx++ )
             {
                 ...
                 {
                     /* Pointer to the ASDM sensor record */
                     ASDMSdrRecord *pxSensorRecordSdr = pxThis->pxAsdmSdrInfo[iRepoIdx].pxSensorRecord;
                     ASDMSdsRecord *pxSensorRecordSds = pxThis->pxAsdmSdsInfo[iRepoIdx].pxSensorRecord;
 ...
 static int iAscCallback( EVL_SIGNAL *pxSignal )
 {
         ...
         ( AMC_CFG_UNIQUE_ID_ASC == pxSignal->ucModule ) )
     {
         ...
         case ASC_PROXY_DRIVER_E_SENSOR_UPDATE_COMPLETE:
             ucNumSensors = pxThis->ucAscNumSensors;
        ...
        for ( int i = APC_BOOT_DEVICE_PRIMARY; i < MAX_APC_BOOT_DEVICES; i++ )
        {
            ...
            if ( OK == iStatus )
            {
                ...
                pxThis->ppxFptPartitions[i] =
                    ( APCProxyDriverFptPartition * )pvOSAL_MemAlloc(
                        pxThis->pxFptHeader[i].ucNumEntries *
                        sizeof( APCProxyDriverFptPartition ) );
 ...
 */
 static int iPopulateAsdmFpt( uint16_t *pusByteCount )
 {
         ...
         ( pusByteCount != NULL ) )
     {
         ...
         usByteCount += sizeof( ASDMFptRecord );

         /* Populate FPT partition information (primary) */
         usAllocateSize = sizeof( ASDMFptEntry ) * pxThis->pxFptHeader[APC_BOOT_DEVICE_PRIMARY].ucNumEntries;
         ...
         {
             iStatus        = ERROR;
             usAllocateSize = sizeof( ASDMFptEntry ) * pxThis->pxFptHeader[APC_BOOT_DEVICE_SECONDARY].ucNumEntries;
 ...
 */
 static int iPopulateAsdmBoardInfo( uint16_t *pusByteCount )
 {
          ...
          ( NULL != pusByteCount ) )
     {
         ...
         *pusByteCount = sizeof( ASDMBoardInfoRecord );
         iStatus       = OK;
     }
     return iStatus;
 }
 ...
 */
 static int iGetBoardInfoData( void )
 {
          ...
          ( NULL == pxThis->pxBoardInfo ) )
     {
         ...
         pxThis->pxBoardInfo = ( ASDMBoardInfoRecord * )pvOSAL_MemAlloc(
             sizeof( ASDMBoardInfoRecord ) );

         if ( NULL != pxThis->pxBoardInfo )
         {
             pvOSAL_MemSet( pxThis->pxBoardInfo, 0, sizeof( ASDMBoardInfoRecord ) );
             ...
             if ( OK == iStatus )
             {
                 ...
                 pxThis->pxBoardInfo->xEepromVersion.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xProductName.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xBoardRev.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xBoardSerial.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xMacAddressCount.ucLength = sizeof( uint8_t ) & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xFirstMacAddress.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xActiveState.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xConfigMode.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xManufacturingDate.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xPartNumber.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xUuid.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xPcieId.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xMaxPowerMode.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xMemorySize.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xOemId.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xCapability.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
             ...
             if ( OK == iStatus )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     pxThis->pxBoardInfo->xMfgPartNumber.ucLength = ucFieldLen & ASDM_RECORD_FIELD_LENGTH_MASK;
 ...
 static int iInitAsdm( uint8_t ucNumSensors )
 {
         ...
         ( NULL == pxThis->pxAsdmSdrInfo ) )
     {
         ...
         pxThis->pxAsdmSdsInfo =
             ( ASDMSds * )pvOSAL_MemAlloc( MAX_ASDM_SDR_REPO * sizeof(ASDMSds) );
         if ( NULL != pxThis->pxAsdmSdrInfo )
         {
             INC_STAT_COUNTER( ASDM_STATS_MALLOC )

             /* Allocate memory for SDR/SDS and sensor list */
             pvOSAL_MemSet( pxThis->pxAsdmSdrInfo, 0, ( MAX_ASDM_SDR_REPO * sizeof(ASDMSdr) ) );
             pvOSAL_MemSet( pxThis->pxAsdmSdsInfo, 0, ( MAX_ASDM_SDR_REPO * sizeof(ASDMSds) ) );
             ...
             for ( iRepoIndex = 0; iRepoIndex < MAX_ASDM_SDR_REPO; iRepoIndex++ )
             {
                 ...
                 if ( OK == iStatus )
                 {
                     ...
                     if ( pxThis->pxSensorList[iRepoIndex].ucNumFound > 0 )
                     {
                         ...
                         case AMC_ASDM_SUPPORTED_REPO_TOTAL_POWER:
                         {
                             usAllocateSizeSdr = sizeof( ASDMSdrRecord ) *
                                                 pxThis->pxAsdmSdrInfo[iRepoIndex].xHdr.ucTotalNumRecords;
                             pxThis->pxAsdmSdrInfo[iRepoIndex].pxSensorRecord =
                                 ( ASDMSdrRecord * )pvOSAL_MemAlloc( usAllocateSizeSdr );

                             usAllocateSizeSds = sizeof( ASDMSdsRecord ) *
                                                 pxThis->pxAsdmSdsInfo[iRepoIndex].xHdr.ucTotalNumRecords;

                             pxThis->pxAsdmSdsInfo[iRepoIndex].pxSensorRecord =
                                 ( ASDMSdsRecord * )pvOSAL_MemAlloc( usAllocateSizeSds );
                             if ( NULL != pxThis->pxAsdmSdsInfo[iRepoIndex].pxSensorRecord )
                             {
                                 ...
                                 ASDMSdrRecord *pxSdr = pxThis->pxAsdmSdrInfo[iRepoIndex].pxSensorRecord;
                                 ASDMSdsRecord *pxSds = pxThis->pxAsdmSdsInfo[iRepoIndex].pxSensorRecord;
                     ...
                     if ( OK == iStatus )
                     {
                         ...
                         /* This is represented in multiple of 8s (i.e.) a record of
                            80 bytes long will be represented as 10 */
                         pxThis->pxAsdmSdrInfo[iRepoIndex].xHdr.usTotalNumBytes =
                ...
                case AMC_ASDM_SUPPORTED_REPO_FPT:
                    if ( 0 < pxThis->pxSensorList[xAsdmRepo].ucNumFound )
                    {
                        ...
                        pvOSAL_MemCpy( &pucRespBuff[usByteCount],
                                       ...
                                       sizeof( ASDMFptHeader ) );
                        usByteCount += sizeof( ASDMFptHeader );

                        /* FPT primary partitions */
                        for ( i = 0; i < pxThis->pxAsdmSdrInfo[xAsdmRepo].xFptRecord.xFptHdrPrimary.ucNumEntries;
                             i++ )
                        {
                            pvOSAL_MemCpy( &pucRespBuff[usByteCount],
                        ...
                        for ( i = 0; i < pxThis->pxAsdmSdrInfo[xAsdmRepo].xFptRecord.xFptHdrSecondary.ucNumEntries;
                             i++ )
                        {
                            pvOSAL_MemCpy( &pucRespBuff[usByteCount],
