...
#define CHECK_FIREWALLS( f )    if( ( f->upperFirewall != UPPER_FIREWALL ) && ( f->lowerFirewall != LOWER_FIREWALL ) ) { INC_ERROR_COUNTER( FW_IF_ERRORS_INVALID_HANDLE_COUNT ); return FW_IF_ERRORS_INVALID_HANDLE; }
...
uint32_t ulFW_IF_UART_Init( FWIfUartInitCfg *pxInitCfg )
{
    ...
    else if ( NULL != pxInitCfg )
    {
         ...
         */
        pvOSAL_MemCpy( &pxThis->xMyLocalCfg, pxInitCfg, sizeof(FWIfUartInitCfg) );
