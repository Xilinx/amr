...
#define CHECK_FIREWALLS( f ) \
    ( ( f->upperFirewall != UPPER_FIREWALL ) && \
      ( f->lowerFirewall != LOWER_FIREWALL ) )

#define CHECK_INVALID_STATE( f ) \
    ( ( FW_IF_GCQ_STATE_OPENED   != ( f )->xState ) && \
...
#define PRINT_STAT_COUNTER( x ) \
     PLL_INF( FW_IF_GCQ_NAME, "%50s . . . . %d\r\n",   \
              FW_IF_GCQ_STATS_STR[ x ],                \
              pxThis->pulStatCounters[ x ] )
#define PRINT_ERROR_COUNTER( x ) \
    PLL_INF( FW_IF_GCQ_NAME, "%50s . . . . %d\r\n",   \
             FW_IF_GCQ_ERRORS_STR[ x ],               \
             pxThis->pulErrorCounters[ x ] )

#define INC_STAT_COUNTER( x ) \
    { if ( x < FW_IF_GCQ_STATS_MAX )pxThis->pulStatCounters[ x ]++; }
#define INC_ERROR_COUNTER( x ) \
    { if ( x < FW_IF_GCQ_ERRORS_MAX )pxThis->pulErrorCounters[ x ]++; }
...
/**
 * @struct  typedef struct FWIfGCQPrivateData
...
typedef struct
{
    ...
    FWIfGCQInitCfg   xLocalCfg;
    GCQIOAccess      xGcqIoAccess;
    int              iInitialised;
    FWIfGCQProfile   xGcqProfile[ GCQ_MAX_INSTANCES ];
...
static FWIfGCQPrivateData xLocalData =
{
...
static FWIfGCQPrivateData *pxThis = &xLocalData;
 ...
 *
 * @param   xError is the return code returned by the gcq driver
 *
 ...
 */
static FW_IF_GCQ_ERRORS_TYPE prvGCQCheckConfig( FWIfCfg *pxFWIf )
{
    ...
    else if ( NULL == pxFWIf->cfg )
    {
        xRet = FW_IF_ERRORS_INVALID_CFG;
        INC_ERROR_COUNTER( FW_IF_ERRORS_INVALID_CFG_COUNT );

    }
    else if ( CHECK_FIREWALLS( pxFWIf ) )
    {
        xRet = FW_IF_ERRORS_INVALID_HANDLE;
        INC_ERROR_COUNTER( FW_IF_ERRORS_INVALID_HANDLE_COUNT );
    }
    else if ( FALSE == pxThis->iInitialised )
    {
...
static uint32_t prvGCQOpen( void *pvFWIf )
{
    ...
    xRet = prvGCQCheckConfig(pxThisIf);

    if ( FW_IF_GCQ_ERRORS_NONE == xRet )
    {
        FWIfGCQCfg *pxCfg = ( FWIfGCQCfg* )pxThisIf->cfg;

        GCQ_MODE_TYPE xMode = prvxMapMode( pxCfg->xMode );

        /* Initially only interrupt polling mode supported */
        FWIfGCQProfile *pxProfile = ( FWIfGCQProfile* )pxCfg->pvProfile;
 ...
 */
static uint32_t prvGCQWrite( void *pvFWIf, uint64_t ullDstPort,
    uint8_t *pucData, uint32_t ulSize, uint32_t ulTimeoutMs )
{
    ...
    if ( FW_IF_GCQ_ERRORS_NONE == xRet )
    {
        ...
        {
            /* vTaskDelay is used outside of function as a block as opposed to a
               spin on the timeout */
            GCQ_ERRORS_TYPE xStatus = xGCQProduceData( pxProfile->pxGCQInstance,
                                                       pucData, ulSize );
 ...
 */
static uint32_t prvGCQRead( void *pvFWIf, uint64_t ullSrcPort, uint8_t *pucData,
    uint32_t *pulSize, uint32_t ulTimeoutMs )
{
    FW_IF_GCQ_ERRORS_TYPE xRet = FW_IF_GCQ_ERRORS_NONE;
    FWIfCfg *pxThisIf = ( FWIfCfg* )pvFWIf;
    ...
    if ( FW_IF_GCQ_ERRORS_NONE == xRet )
    {
        ...
        else
        {
            /* vTaskDelay is used outside of function as a block as opposed to a
            spin on the timeout */
            GCQ_ERRORS_TYPE xStatus = xGCQConsumeData( pxProfile->pxGCQInstance,
                                                       pucData, *pulSize );
...
/**
 * @brief   initialisation function for sGCQ interfaces (generic across all
 *          sGCQ interfaces)
 */
uint32_t ulFW_IF_GCQ_Init( FWIfGCQInitCfg *pxCfg )
{
...
uint32_t ulFW_IF_GCQ_Create( FWIfCfg *pxFWIf, FWIfGCQCfg *pxGCQCfg )
{
         ...
         ( TRUE == pxThis->iInitialised ) )
    {
        ...
        if ( FW_IF_GCQ_ERRORS_NONE == xRet )
        {
            ...
            if ( ( GCQ_MAX_INSTANCES - 1 ) < pxThis->ulProfilesAllocated )
            {
