...
#define CHECK_ATTACHED(f)	((GCQ_MODE_TYPE_CONSUMER_MODE == f->xMode) && \
 ...
 * @return  N/A
 */
static inline void prvvGCQFastForward(const GCQ_INSTANCE_TYPE *pxGCQInstance,
	struct GCQ_RING_TYPE *pxRing)
{
    gcq_assert(pxGCQInstance);
    gcq_assert(pxRing);

    prvvGCQRingReadProduced(pxGCQInstance->pxGCQIOAccess,
		pxGCQInstance->xGCQFlags,
		pxRing);
    prvvGCQRingReadConsumed(pxGCQInstance->pxGCQIOAccess,
		pxGCQInstance->xGCQFlags,
		pxRing);
}
 ...
 */
static inline bool prvucGCQCanProduce(const GCQ_INSTANCE_TYPE *pxGCQInstance,
	struct GCQ_RING_TYPE *pxRing)
{
    bool ulStatus = false;
 ...
 */
static inline bool prvucGCQCanConsume(const GCQ_INSTANCE_TYPE *pxGCQInstance,
	struct GCQ_RING_TYPE *pxRing)
{
    bool ulStatus = false;
 ...
 * @return  N/A
 */
static inline void prvvGCQSoftReset(const GCQ_INSTANCE_TYPE *pxGCQInstance,
		struct GCQ_RING_TYPE *pxRing)
{
    gcq_assert(pxGCQInstance);
    gcq_assert(pxRing);
    ...
    prvvGCQRingWriteConsumed(pxGCQInstance->pxGCQIOAccess,
		pxGCQInstance->xGCQFlags,
		pxRing);
}
 ...
 */
static inline GCQ_ERRORS_TYPE prvxGCQConsume(GCQ_INSTANCE_TYPE *pxGCQInstance,
	uint64_t *ullSlotAddr)
{
    GCQ_ERRORS_TYPE xStatus = GCQ_ERRORS_NONE;
...
static inline GCQ_ERRORS_TYPE prviGCQFindNextFreeInstance(
	struct GCQ_INSTANCE_TYPE **ppxGCQInstance)
{
                          ...
                          uint32_t ulCQSlotSize)
{
    ...
    if ((GCQ_INSTANCE_UPPER_FIREWALL == pxThis->ulUpperFirewall) &&
        ...
        if (GCQ_ERRORS_NONE == xStatus) {
            ...
            (*ppxGCQInstance)->iInitialised = false;
            ...
            (*ppxGCQInstance)->iInitialised = true;
