...
def get_git_info(id):
    ...
    try:
       ...
       exit_error(id, 'Warning: Could not get git info: {e}')

def append_git_to_makefile(id, makefile):
    """Append Git hash and date to CFLAGS in Makefile."""

    # Get Git information
    git_hash, git_date = get_git_info(id)

    # Git defines to add
    git_defines = f'-DGIT_HASH=\\"{git_hash}\\" -DGIT_DATE=\\"{git_date}\\"'

    # Read Makefile
    with open(makefile, 'r') as f:
        lines = f.readlines()

    # Find and modify CFLAGS line
    modified = False
    for i, line in enumerate(lines):
        # Match CFLAGS line (handle various formats)
        if re.match(r'^CFLAGS\s*[:+]?=', line):
            # Remove trailing newline
            line = line.rstrip('\n')
            # Append git defines
            lines[i] = f"{line} {git_defines}\n"
            modified = True
            break

    if not modified:
        print("Warning: CFLAGS line not found in Makefile")
        return False
